...
if (changedFiles.length === 0) {
  if (isManual) {
    console.log(
      "Ejecución manual: documentando una muestra del proyecto completo.",
    );
    changedFiles = sampleProjectFiles();
  } else {
    console.log(
      "Push sin cambios de código (.ts/.tsx/.js). Omitiendo generación.",
    );
    process.exit(0);
  }
}
...
if (changedFiles.length === 0) {
  console.log(
    "No se encontraron ficheros de código para documentar. Omitiendo.",
  );
  process.exit(0);
}
...
try {
  srcTree = execSync(`find ${srcDir} -type f | sort | head -60`, { cwd: ROOT })
    .toString()
    .trim();
} catch {
  /* noop */
  ...
  for (let attempt = 1; attempt <= 4; attempt++) {
    try {
      ...
      if ((status === 429 || status >= 500) && attempt < 4) {
        ...
        console.log(
          `  Reintento ${attempt}/3 tras ${status} (espera ${wait}ms)…`,
        );
...
console.log(
  `Generando docs para ${REPO}@${SHORT_SHA} (${changedFiles.length} ficheros)…`,
);
...
try {
  ...
  slides = [
    {
      title: pkg.name || repoShort,
      bullets: [
        "Documentación técnica generada automáticamente con Mistral AI",
      ],
      notes: "",
    },
    {
      title: "Tech Stack",
      bullets: Object.keys(pkg.dependencies || {}).slice(0, 6),
      notes: "",
    },
  ];
}
...
slides.forEach((slide, i) => {
  ...
  const bullets = Array.isArray(slide.bullets)
    ? slide.bullets.filter(Boolean).map(String)
    : [];
  ...
  if (i === 0) {
    ...
    s.addShape(prs.ShapeType.ellipse, {
      ...
      w: 5.6,
      h: 5.6,
      fill: { color: C.accent, transparency: 55 },
      line: { type: "none" },
    });
    s.addShape(prs.ShapeType.ellipse, {
      x: 11.2,
    ...
    // Eyebrow
    s.addText("DOCUMENTACIÓN TÉCNICA", {
      x: 0.9,
      y: 2.25,
      w: 10,
      h: 0.4,
    ...
    s.addText(`${REPO}  ·  commit ${SHORT_SHA}`, {
      x: 0.9,
      y: 6.65,
      w: 11,
      h: 0.4,
  ...
  s.addShape(prs.ShapeType.rect, {
    ...
    y: 2.05,
    w: 0.07,
    h: Math.min(4.3, 0.55 * bullets.length + 0.3),
    fill: { color: C.tint },
    line: { type: "none" },
  });
  ...
  s.addText(`${num} / ${String(total - 1).padStart(2, "0")}`, {
    x: PAGE_W - 2.6,
    y: 6.98,
    w: 2,
    h: 0.35,
...
<body style="font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;max-width:860px;margin:0 auto;padding:24px;background:#f1f5f9;color:#1e293b">
  ...
  <div style="background:white;border:1px solid #e2e8f0;border-top:none;padding:28px 32px;border-radius:0 0 12px 12px">
    ...
    <div style="background:#f8fafc;border:1px solid #e2e8f0;border-radius:8px;padding:16px;overflow-x:auto">
      <pre style="margin:0;font-size:12px;color:#334155;white-space:pre-wrap;font-family:'Menlo','Monaco','Courier New',monospace;line-height:1.6">${escapeHtml(readme)}</pre>
    </div>
    ...
    <div style="background:#f8fafc;border:1px solid #e2e8f0;border-radius:8px;padding:16px;overflow-x:auto">
      <pre style="margin:0;font-size:12px;color:#334155;white-space:pre-wrap;font-family:'Menlo','Monaco','Courier New',monospace;line-height:1.6">${escapeHtml(funcDocs)}</pre>
    </div>
